...
function launchBrowser() launch(lastBrowser) end
    ...
    for _, browser in ipairs(browsers) do
        if browser[1] == appName then
            if appName ~= lastBrowser then
                print("changing browser from " .. lastBrowser .. " to " ..
                          appName)
                lastBrowser = appName
            end
...
function httpCallback(scheme, _, _, fullURL)
    ...
    local asanaRegex = "^https://app.asana.com/"
    ...
    elseif fullURL:match(zoomWithPasswordRegex) then
        fullURL = fullURL:gsub(zoomWithPasswordRegex,
    ...
    elseif fullURL:match(asanaRegex) then
        ...
        for _, browser in ipairs(browsers) do
            if browser[1] == lastBrowser then
                ...
                local isHammerspoon = frontmostApp:bundleID() ==
